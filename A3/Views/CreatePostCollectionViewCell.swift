...
class CreatePostCollectionViewCell: UICollectionViewCell {
    ...
    @objc private func createPost() {
        ...
        guard let message = textField.text, !message.isEmpty else {
            print("Post message is empty.")
            return
        }

            // Call addPost from NetworkManager
        NetworkManager.shared.addPost(message: message) { [weak self] newPost in
            guard let self = self else { return }
            print("Succesfully added post!")
            
            // MARK: - TODO: remove text from the textField to show user their post went through
        }
    }
